# Balbec Capital Analyst Case Study
# For any inquiries, please reach out to thomasjditsworth@gmail.com

import pandas as pd


# Function that answers the inital question on the first Excel sheet
def question1(q1_filepath, excel_file_path):
    column_6th = q1_filepath.iloc[:, 5]
    column_7th = q1_filepath.iloc[:, 6]
    right_table_dict = {}

    # Adding all values from the Right table into the dictionary
    ...
    # Adding the matching Asset Type to the Right Table
    ...
    # Using ExcelWriter to append to the original Excel file
    with pd.ExcelWriter(excel_file_path, engine='openpyxl', mode='a') as writer:
        ...
        # Writing the DataFrame to the existing sheet
        q1_filepath.to_excel(writer, sheet_name='Question 1', index=False, header=False)


# This is an attempt to produce the incorrect methodology originally used on Question 2
def question2_incorrect(q2_filepath, excel_file_path):
    # Here, I used nested for loops to simulate iterating over the wrong data in a
    # way that this type of mistake is believeable
    for x in range(6, 13):
    ...
    # The same method as above to update the table
    with pd.ExcelWriter(excel_file_path, engine='openpyxl', mode='a') as writer:
        try:
...
# This is my actual attempt at question 2
# It uses the same functionality as question 1
...
# My solution to Question 3 is broken up into helper functions to avoid overloading this method
# Due to the nature of the question, each one of these helper functions operates in the same manner
...
def question3_helper_country(q3_filepath, excel_file_path):
    # creating variables to store the sums
    ...
    # Designating the columns I will be iterating over
    ...
    # Updating the sums on the Excel Sheet
...
def question3_helper_type(q3_filepath, excel_file_path):
    # Creating variables to store the sum
    ...
    # Designating the columns to iterate over
    column_j = q3_filepath.iloc[:, 9]
    column_i = q3_filepath.iloc[:, 8]
    column_k = q3_filepath.iloc[:, 10]

    # Adding the values to the summation variables
...
def main():
    ...
    print("The code has finished! Thank you for your time - Tom Ditsworth")
